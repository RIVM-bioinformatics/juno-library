...
def test_real_nanopore_output_sample_dict():
    ...
    pipeline = Pipeline(
        ...
        input_type="fastq",
        argv=[
            "--input", str(real_nanopore_dir),
            "--output", str(real_nanopore_dir / "output"),
            "--sequencing-tech", "nanopore"
        ]
    )
    pipeline.setup()
    print("Sample dict:")
...
def main():
    ...
    parser.add_argument('--tech', choices=['illumina', 'nanopore', 'both', 'nanopore_output'], required=True, help="Specify the sequencing technology to test.")
