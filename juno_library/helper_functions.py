...
class FileHelpers:
    ...
    def validate_is_nonempty_file(self, file_path, min_file_size=0):
        ...
        nonempty_file = (
            file_path.is_file() and file_path.stat().st_size >= min_file_size
        )
        if nonempty_file:
            return True
        else:
    ...
    def download_git_repo(self, version, url, dest_dir):
        ...
        try:
            ...
            rm_dir = subprocess.run(
                ["rm", "-rf", str(dest_dir)], check=True, timeout=60
            )
...
class SnakemakeKwargsAction(argparse.Action, JunoHelpers):
    """
